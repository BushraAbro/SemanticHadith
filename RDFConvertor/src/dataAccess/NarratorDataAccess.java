...
public class NarratorDataAccess {
	...
	public static Raavi setNarratorAtt(int Index,Connection conn, Statement st){
		...
		try {
			ResultSet s = st.executeQuery("SELECT `Travi`, `hadith_number`,`bookschapters_id`,`bookssubchapters_id` FROM csb_hadith  WHERE `hadith_number` NOT LIKE 'Q%' AND hadith_id="+Index);
			while(s.next()){
