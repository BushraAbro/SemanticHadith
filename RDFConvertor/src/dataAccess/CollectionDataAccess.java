...
public class CollectionDataAccess {
	...
	public static CollectionData setCollectionAtt(int Index,Connection conn, Statement st){
		...
		try {
			ResultSet s = st.executeQuery("SELECT `arabic_s`,`urdu`,`english`,`booksnames_id` FROM 02_booksnames where `booksnames_id`="+Index);
			while(s.next()){
			...
			collection.setCollectionID(Integer.parseInt(s.getString(4)));
